...
-(void)setParallaxIntensity:(CGFloat)parallaxDepth
{
    ...
    if (!parallaxGroup)
    {
        parallaxGroup = [[[UIMotionEffectGroup alloc] init] autorelease];
        [self nga_setParallaxMotionEffectGroup:parallaxGroup];
        [self addMotionEffect:parallaxGroup];
    }
    
    UIInterpolatingMotionEffect *xAxis = [[[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.x" type:UIInterpolatingMotionEffectTypeTiltAlongHorizontalAxis] autorelease];
    UIInterpolatingMotionEffect *yAxis = [[[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:UIInterpolatingMotionEffectTypeTiltAlongVerticalAxis] autorelease];
