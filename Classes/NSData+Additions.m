 ...
 * @changed $Id: NSData+Additions.m 220 2013-02-02 16:21:28Z mkovachev $
...
@implementation NSData(LAdditions)
...
- (unsigned int)crc32
{
	...
	crcval = 0xffffffff;
	for (x = 0, y = max; x < y; x++) {
		crcval = ((crcval >> 8) & 0x00ffffff) ^ (unsigned int)crc32table[(crcval ^ (*((unsigned char *)bytes + x))) & 0xff];
	}
