 ...
 * @changed $Id: CTGradient.m 272 2013-06-21 10:46:41Z mkovachev $
...
+ (id)gradientWithBeginningColor:(NSColor *)begin endingColor:(NSColor *)end
{
	...
	@finally 
	{
		newInstance = [newInstance autorelease];
	}
...
- (void)addElement:(CTGradientElement *)newElement
{
		...
		// inserting somewhere inside list
	{
		...
		while(curElement->nextElement != nil && !((curElement->position <= newElement->position) && 
...
void chromaticEvaluation(void *info, const float *in, float *out)
{
	...
	// if color1's hue is higher than color2's hue then 
	// we need to move c2 one revolution around the wheel
	if(c1[0] > c2[0]) 
	{
		c2[0] += 360;	
	}
...
void inverseChromaticEvaluation(void *info, const float *in, float *out)
{
	...
	//if color1's hue is higher than color2's hue then 
	//	we need to move c2 one revolution back on the wheel
	if(c1[0] < c2[0]) 
	{
		c1[0] += 360;
	}
...
void transformHSV_RGB(float *components) 
{
	...
	//map to [0,360)
	float H = fmodf(components[0],359),	
	S = components[1],
	V = components[2];
...
//H value may be undefined (i.e. graycale color)
void resolveHSV(float *color1, float *color2)	
{											
